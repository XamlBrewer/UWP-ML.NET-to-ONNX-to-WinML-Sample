...
namespace Regression_TaxiFarePrediction
{
    internal static class Program
    {
        ...
        private static ITransformer BuildTrainEvaluateAndSaveModel(MLContext mlContext)
        {
            ...
            // XAML Brewer Addendum

            // STEP 7: Export the trained model to a .ONNX file
            var onnxModelRelativePath = $"{BaseModelsRelativePath}/TaxiFareModel.onnx";
            var onnxModelPath = GetAbsolutePath(onnxModelRelativePath);
            using (var stream = File.Create(onnxModelPath))
            {
                mlContext.Model.ConvertToOnnx(trainedModel, trainingDataView, stream);
            }

            Console.WriteLine("The model is exported to {0}", onnxModelPath);

            // End of XAML Brewer Addendum
